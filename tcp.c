...
tcp_acceptor *create_tcp_acceptor(int port) {
  ...
  acceptor->addr.sin_addr.s_addr = htonl(INADDR_ANY);// /htonl(INADDR_LOOPBACK);/ set this if you want to accept any address rather than just addresses on this machine
