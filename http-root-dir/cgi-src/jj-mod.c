...
void print_error(char *reason, FILE *fssock)
{
...
extern void httprun(int ssock, char *query_string) {
    ...
    fprintf(fssock, "HTTP/1.1 200 Document follows%c%cServer: Server-Type%c%c", CR, LF, CR, LF);
    ...
    if (!name[0])
        print_error("you didn't give your name", fssock);
    if (!address[0])
        print_error("you didn't give your address", fssock);
    if (!phone[0])
        print_error("you didn't give your phone number", fssock);
    if ((!subs) && (!slims) && (!sides) && (!drinks))
        print_error("you didn't order anything", fssock);
    ...
    if (allow)
    {
        ...
        if (!(order = popen(t, "w")))
            print_error("the server was unable to open a pipe to mail", fssock);
